...
public class ActiveSnapshotManager : IActiveSnapshotManager
{
    ...
    public void OnGPoseEntered()
    {
        ...
        if (localPlayerSnapshot != null)
        {
            ...
            PluginLog.Debug(
                $"Updated local player snapshot tracking from index {localPlayerSnapshot.ObjectIndex} to GPose index {GameAddresses.GPoseObjectId}");
        }
    }
    ...
    {
        var gposeSnapshot =
            _activeSnapshots.FirstOrDefault(s => s.ObjectIndex == GameAddresses.GPoseObjectId && s.IsOnLocalPlayer);
        if (gposeSnapshot != null && Player.Available)
        {
            ...
            PluginLog.Debug(
                $"Updated GPose snapshot tracking from index {GameAddresses.GPoseObjectId} to regular player index {Player.Object.ObjectIndex}");
        }
    }
    ...
    private static bool IsMatchingLocalPlayer(ActiveSnapshot snapshot, ICharacter character)
    {
        ...
        if (snapshot.ObjectIndex == GameAddresses.GPoseObjectId && character.Address == Player.Object.Address)
            return true;
